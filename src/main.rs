...
impl UpdateGitLocal {
    async fn init() -> Self {
        ...
        time.push('\n');
        let d = time.as_bytes();
        UpdateGitLocal::write_logs(d).await;

        // path for works with
        let path = current_dir().unwrap();
        
        let nested_dirs = UpdateGitLocal::dirs(path);
        ...
        for i in self.nested_dirs {
            tasks.push(tokio::spawn(async move {
                ...
                if !out.status.success() {
                    UpdateGitLocal::write_logs(out.stderr.as_slice()).await;
                } else {
                    ...
                    match result {
                        Some(a) if a.contains("Уже актуально") => {
                            ...
                            UpdateGitLocal::write_logs([data, b" -> ",out.stdout.as_slice()].concat().as_slice()).await;
                        }
                        ...
                        None => {
                            ...
                            if result.is_none() {
                                ...
                                UpdateGitLocal::write_logs([data.as_bytes(), b" -> ",out.stdout.as_slice()].concat().as_slice()).await;
...
#[tokio::main]
async fn main() {
    let update = UpdateGitLocal::init().await;
